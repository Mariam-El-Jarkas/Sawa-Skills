...
export function useSwaps(): SwapsState & SwapsActions {
  ...
  const rateSwap = useCallback(async (id: number, data: RatingData) => {
    ...
    // Optimistically update status to completed after rating
    setSwaps(prev => prev.map(s => s.id === id ? { ...s, status: 'completed' as const } : s));
  }, [token]);

  return { swaps, isLoading, error, fetchSwaps, createSwap, acceptSwap, rejectSwap, rateSwap };
}

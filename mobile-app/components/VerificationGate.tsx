...
import { ShieldCheck, CreditCard, Baby, ChevronRight } from 'lucide-react-native';
...
export function VerificationGate({ feature }: Props) {
  ...
  const goVerify = (type: 'adult' | 'minor') =>
    router.push(`/profile/${user?.id}?verify=${type}`);

  const iconColor  = isDark ? '#C4B5FD' : '#7C3AED';
  const chevronColor = isDark ? 'rgba(196,181,253,0.45)' : '#8B5CF6';

  const Opt = ({ type, Icon, title, sub }: { type: 'adult' | 'minor'; Icon: any; title: string; sub: string }) => (
    <TouchableOpacity onPress={() => goVerify(type)} activeOpacity={0.7}>
      <View style={s.opt}>
          ...
          <View style={s.inner}>
            ...
            <View style={s.iconHalo}>
              <View style={s.iconCore}>
                <ShieldCheck size={28} color={iconColor} strokeWidth={2.2} />
              </View>
            </View>

            <View style={s.pill}>
              <ShieldCheck size={10} color={isDark ? '#C4B5FD' : '#6D28D9'} strokeWidth={2.5} />
              <Text style={s.pillTxt}>{feature.toUpperCase()} · LOCKED</Text>
            </View>

            <Text style={s.title}>Verify to Unlock</Text>
            <Text style={s.sub}>
              This feature is only available to verified users. Choose how you'd like to get verified.
            </Text>

            <View style={s.divRow}>
              <View style={s.divLine} />
              <Text style={s.divTxt}>CHOOSE VERIFICATION</Text>
              <View style={s.divLine} />
            </View>

            <View style={s.options}>
              <Opt type="adult" Icon={CreditCard} title="I'm an Adult (16+)"     sub="Upload your ID for full access" />
              <Opt type="minor" Icon={Baby}       title="I'm a Minor (under 16)" sub="A parent approves your account" />
            </View>
          </View>
        </Animated.View>
      </View>
